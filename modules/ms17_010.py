...
def check(ip, port=445):
  ...
  try:
    ...
    if native_os=="Windows 5.1":
            native_os="Windows XP"
    ...
    if nt_status == '\x05\x02\x00\xc0':
      ...
      result = f"[+] [{ip}] is likely VULNERABLE to MS17-010! ({native_os})"
      ...
      if smb.multiplex_id == 0x0051:
        ...
        result = f"Host is likely INFECTED with DoublePulsar! - XOR Key: {key}"

    elif nt_status in ('\x08\x00\x00\xc0', '\x22\x00\x00\xc0'):
      result = f"[-] [{ip}] does NOT appear vulnerable"
    else:
      result = f"[-] [{ip}] Unable to detect if this host is vulnerable"
    return result
